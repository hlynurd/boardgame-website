...
fetch('https://uhran-gamers-python.ew.r.appspot.com/plot_data/', {  // Make sure the URL matches the Flask route exactly, including trailing slash if needed
    method: 'POST',  // Specify the method, especially if your route handles different methods differently
    headers: {
        'Content-Type': 'application/json',  // Ensuring the content type is set correctly for JSON data
        'Accept': 'application/json',  // Explicitly declare what type of responses are acceptable
        'X-Custom-Header': 'value'  // Example of a custom header; replace 'X-Custom-Header' and 'value' with what you might need
    },
    body: JSON.stringify({  // Only necessary if you're sending data with the request
        // Your request body here
    })
})
    .then(response => response.json())
    .then(data => {
        const maxDataValue = Math.max(...data.datasets.map(ds => Math.max(...ds.data))) * 1.01; // 1% more than max
        const minDataValue = Math.min(...data.datasets.map(ds => Math.min(...ds.data))) * 0.99; 

        const originalColors = data.datasets.map(ds => ({borderColor: ds.borderColor, backgroundColor: ds.backgroundColor})); // Store original colors and background
        const myChart = new Chart(ctx, {
            ...
            options: {
                scales: {
                    ...
                    x: {
                        autoSkip: true,
                        maxTicksLimit: data.labels.length / 2,
                        grid: {
                            display: false
                        },
                        ticks: {
                ...
                plugins: {
                    ...
                    legend: {
                      onHover: function(event, legendItem) {
                        ...
                        chart.data.datasets.forEach((e, i) => {
                            e.borderColor = i === index ? originalColors[i].borderColor : 'rgba(128, 128, 128, 0.3)';
                            e.backgroundColor = i === index ? originalColors[i].backgroundColor : 'rgba(128, 128, 128, 0.1)';
                        });
                        chart.update();
                    },
                    onLeave: function(event, legendItem) {
                        ...
                        chart.data.datasets.forEach((e, i) => {
                            e.borderColor = originalColors[i].borderColor;
                            e.backgroundColor = originalColors[i].backgroundColor;
                        });
                        chart.update();
                        }
                    }
